...
if (empty($arRunErrors))
{
	...
	if ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_CATALOG || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_OFFERS)
	{
		$arSelect = array(
			...
			"PREVIEW_PICTURE", "PREVIEW_TEXT",  "DETAIL_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL",
			"CATALOG_AVAILABLE"
		);
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'name':
					...
					break;
				case 'description':
					 $arAcc["~PREVIEW_TEXT"]= $arAcc["~PREVIEW_TEXT"]? $arAcc["~PREVIEW_TEXT"]: $arAcc["DETAIL_TEXT"];
					$strTmpOff .=
						"<description>".
						yandex_text2xml(
							//TruncateText(
							//($arAcc["PREVIEW_TEXT_TYPE"]=="html"?
							strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])), 
							//: preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])),
							//255), 
						true).
						"</description>\n";
					break;
	...
	elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
	{
		...
		while ($obItem = $rsItems->GetNextElement())
		{
			...
			$arItem["~PREVIEW_TEXT"]=$arItem["~PREVIEW_TEXT"]?$arItem["~PREVIEW_TEXT"]:$arItem["~DETAIL_TEXT"];
			$arItem['YANDEX_DESCR'] = yandex_text2xml(
				//TruncateText(
							//($arItem["PREVIEW_TEXT_TYPE"]=="html"?
							strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])), 
							//:preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])),
			//				255), 
			true);
						...
						foreach ($arYandexFields as $key)
						{
							switch ($key)
							{
								case 'name':
									...
									break;
								case 'description':
									$arOfferItem["~PREVIEW_TEXT"]=$arOfferItem["~PREVIEW_TEXT"]?$arOfferItem["~PREVIEW_TEXT"]:$arOfferItem["~DETAIL_TEXT"];
									...
									else
									{
										$strOfferYandex .= yandex_text2xml(
											...
											//($arOfferItem["PREVIEW_TEXT_TYPE"] == "html" ?
												strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])), 
												//: $arOfferItem["~PREVIEW_TEXT"]),
											//255),
											true);
									}
									$strOfferYandex .= "</description>\n";
							...
							switch ($key)
							{
								...
								case 'description':
									...
									else
									{
										...
										//	($arOfferItem["PREVIEW_TEXT_TYPE"] == "html" ?
												strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])),
												// : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])),
											//255),
											true);
									}
									$strOfferYandex .= "</description>\n";
