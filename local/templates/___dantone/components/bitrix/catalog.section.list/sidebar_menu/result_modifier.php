...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	if ('LINE' == $arParams['VIEW_MODE'] || 'TILE' == $arParams['VIEW_MODE'])
	{
		...
		{
			if($arParams['LANGUAGE_ID']=='en' && $arSection['UF_ENG_NAME'])$arResult['SECTIONS'][$key]['NAME']=$arSection['UF_ENG_NAME'];
		}
	...
	foreach ($arResult['SECTIONS'] as $key => &$arSection) {
		    ...
		    if(in_array($arSection["CODE"], $sections)) {
			    $arSection["ACTIVE_SECTION"] = "Y";
		    }
		    
	        $arFilter = array('IBLOCK_ID' => $arSection["IBLOCK_ID"], "SECTION_ID" => $arSection["ID"], "ACTIVE" => "Y");
			$arSelect = array("ID", "CODE", "NAME", "SECTION_PAGE_URL", "DEPTH_LEVEL");
			
			
			$subsections = CIBlockSection::GetList(Array("SORT" => "ASC"), $arFilter, true, $arSelect);
			
			$arSection["SUBSECTIONS"] = Array();
			
			
			while($arSubsection = $subsections->GetNext()) {
				$arSection["SUBSECTIONS"][] = $arSubsection;
			}
			
			foreach($arSection["SUBSECTIONS"] as &$subsection) {
				if(in_array($subsection["CODE"], $sections)) {
				    $subsection["ACTIVE_SECTION"] = "Y";
			    }
			}
