...
if($_SESSION["PRICE_DELIVERY"] > 0 || $arResult["ORDER"]["PRICE_DELIVERY"] == 0)
{
	$arResult["ORDER"]["PRICE"] = $_SESSION["PRICE_DELIVERY"] + $arOrder["PRICE"];
	$arResult["ORDER"]["PRICE_DELIVERY"] = $_SESSION["PRICE_DELIVERY"];
	$arFields = array("PRICE_DELIVERY" => $_SESSION["PRICE_DELIVERY"], "PRICE"=>$arResult["ORDER"]["PRICE"]);
	CSaleOrder::Update($arResult["ORDER"]["ID"], $arFields);
	$_SESSION["PRICE_DELIVERY"] = 0;

}

$fn = $_SERVER["DOCUMENT_ROOT"]."/log1.txt";
$hp = fopen($fn, "a+");

fwrite($hp, " [PRICE_DELIVERY => ".$arResult["ORDER"]["PRICE_DELIVERY"]."] ");
fwrite($hp, " [PRICE => ".$arResult["ORDER"]["PRICE"]."] ");
fwrite($hp, " [DELIVERY_ID => ".$arResult["ORDER"]["DELIVERY_ID"]."] ");
fwrite($hp, " [PAY_SYSTEM_ID => ".$arResult["ORDER"]["PAY_SYSTEM_ID"]."] ");
