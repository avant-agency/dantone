...
$rsFirstElement = CIBlockElement::GetList($arSort, $arFilter, false, false, $arSelect);
$rsFirstElement->SetUrlTemplates($arParams["DETAIL_URL"]);
while($obFirstElement = $rsFirstElement->GetNextElement())
	$arFirstItems[] = $obFirstElement->GetFields();

$arResult["FIRST_ITEM"] = Array("NAME"=>$arFirstItems[count($arFirstItems) - 1]["NAME"], "URL"=>$arFirstItems[count($arFirstItems) - 1]["DETAIL_PAGE_URL"]);	
$arResult["LAST_ITEM"] = Array("NAME"=>$arFirstItems[0]["NAME"], "URL"=>$arFirstItems[0]["DETAIL_PAGE_URL"]);		
...
elseif(count($arItems)==2):
	...
	if (!empty($arID))
	{
		...
		while ($arItems = $dbBasketItems->Fetch())
		{
			...
			if($arItems["PRODUCT_ID"] == $arResult["OFFERS"][0]["ID"]) {
	...
	if($arResult['IBLOCK_SECTION_ID']){
		$nav = CIBlockSection::GetNavChain($arResult['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], array('ID'));
		while($arSection = $nav->Fetch()){
			$ids[]=$arSection['ID'];
		}
		$arResult['RECOMMENDS'] = array();
		$arResult['RECOMMENDS_IDS'] =array();
		$nav = CIBlockSection::GetList(Array("SORT"=>"­­ASC"), Array("IBLOCK_ID" => $arResult['IBLOCK_ID'], "ID" => $ids), false, array('ID', 'UF_RECOMMENDS','UF_RECOMMENDS_ITEMS'));
		while($arSection = $nav->GetNext()){
			if($arSection['UF_RECOMMENDS'])$arResult['RECOMMENDS']=array_merge($arResult['RECOMMENDS'], $arSection['UF_RECOMMENDS']);
			if($arSection['UF_RECOMMENDS_ITEMS'])$arResult['RECOMMENDS_IDS']=array_merge($arResult['RECOMMENDS_IDS'], $arSection['UF_RECOMMENDS_ITEMS']);

		}
		
	}
	if($arParams['LANGUAGE_ID']=='en'){
		if($arResult['SECTION']['PATH']){
			...
			$dbRes = CIBlockSection::GetList(Array(), Array(
				...
				"ID" => array_keys($ids)
				), false, array('ID','UF_ENG_NAME', 'UF_ENG_DESCRIPTION'));
			while($arSection = $dbRes->GetNext()){
				$arPath=array();
				if($arSection['UF_ENG_NAME']){
					$arResult['SECTION']['PATH'][$ids[$arSection['ID']]]['NAME']=$arSection['UF_ENG_NAME'];
				}
				if($arSection['UF_ENG_DESCRIPTION']){
					$arResult['SECTION']['PATH'][$ids[$arSection['ID']]]['DESCRIPTION']=$arSection['UF_ENG_DESCRIPTION'];
				}
			}
		}else{
			$dbRes = CIBlockSection::GetList(Array(), Array(
			...
			if($arSection = $dbRes->GetNext()){
				$arPath=array();
				if($arSection['UF_ENG_NAME']){
					$arResult['SECTION']['NAME']=$arSection['UF_ENG_NAME'];
				}
				if($arSection['UF_ENG_DESCRIPTION']){
					$arResult['SECTION']['DESCRIPTION']=$arSection['UF_ENG_DESCRIPTION'];
				}
			}
		}
		if($arResult['PROPERTIES']['ENG_NAME']['VALUE']){
			$arResult['NAME']=$arResult['PROPERTIES']['ENG_NAME']['VALUE'];
		}
		if($arResult['PROPERTIES']['ENG_DETAIL_TEXT']['VALUE']){
			$arResult['DETAIL_TEXT']=$arResult['PROPERTIES']['ENG_DETAIL_TEXT']['~VALUE']['TEXT'];
		}
		if($arResult["PROPERTIES"]["ENG_FEATURES"]["VALUE"]){
			$arResult["PROPERTIES"]["FEATURES"]=$arResult['PROPERTIES']['ENG_FEATURES'];
		}

		
	}	
	$this->__component->SetResultCacheKeys(array('NAME', 'DETAIL_TEXT', 'SECTION', 'RECOMMENDS', 'RECOMMENDS_IDS'));
...
$dbBasketItems = CSaleBasket::GetList(
	...
	array("FUSER_ID" => CSaleBasket::GetBasketUserID(), "LID" => "s1", "ORDER_ID" => "NULL", "CAN_BUY" => "Y"),
...
while ($arItems = $dbBasketItems->Fetch())
{
	...
	if (is_array($arInfo)) {
    	...
    	while ($arOffer = $rsOffers->GetNext()) {
			if($arOffer["ID"] == $arItems["PRODUCT_ID"])
			{
				$arResult["CART_TEXT"] = "В корзине " . $arItems["QUANTITY"] . " шт.<br/>перейти";
				$arResult["COUNT_IN_CART"] = $arItems["QUANTITY"];
			}
    	}
	}
	else
