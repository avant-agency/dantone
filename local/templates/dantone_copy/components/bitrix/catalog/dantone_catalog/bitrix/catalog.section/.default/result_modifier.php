...
$rsOrder = Array("SORT" => "ASC");
$rsFilter = Array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ID" => $arResult["ID"]);
$rsSelect = Array("UF_SLIDERPICS");
$rsSection = CIBlockSection::GetList($rsOrder,$rsFilter, true, $rsSelect); 

while($arSect = $rsSection->GetNext()) {

	$arResult["UF_SLIDERPICS"] = $arSect["UF_SLIDERPICS"];
...
if($arParams['LANGUAGE_ID']=='en'){
	if($arResult['ID']>0){
		foreach($arResult['PATH'] as $key=>$arPath){
			$ids[$arPath['ID']]=$key;
		}
		...
		while($arSection = $dbRes->GetNext()){
			$arPath=array();
			if($arSection['ID']==$arResult['ID'])$arSection['RESULT']=1;
			if($arSection['UF_ENG_NAME']){
				if($arSection['RESULT'])$arResult['NAME']=$arSection['UF_ENG_NAME'];
				$arResult['PATH'][$ids[$arSection['ID']]]['NAME']=$arSection['UF_ENG_NAME'];
			}
			if($arSection['UF_ENG_DESCRIPTION']){
				if($arSection['RESULT'])$arResult['DESCRIPTION']=$arSection['~UF_ENG_DESCRIPTION'];
				$arResult['PATH'][$ids[$arSection['ID']]]['DESCRIPTION']=$arSection['~UF_ENG_DESCRIPTION'];
			}
		}
		$this->__component->SetResultCacheKeys(array('NAME', 'DESCRIPTION', 'PATH'));
	}	
	foreach($arResult['ITEMS'] as $key=>$arItem){
...
if($arResult['ID']){
		$nav = CIBlockSection::GetNavChain($arResult['IBLOCK_ID'], $arResult['ID'], array('ID'));
