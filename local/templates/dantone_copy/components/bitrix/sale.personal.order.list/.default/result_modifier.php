...
<?
	...
	if($arParams['LANGUAGE_ID']==LANGUAGE_ID){
		foreach($arResult["INFO"]["STATUS"] as $id=>$arStatus){
			if($id=='F')$arResult["INFO"]["STATUS"][$id]['NAME']='Complete';
				elseif($id=='N')$arResult["INFO"]["STATUS"][$id]['NAME']='Pending payment';
				elseif($id=='P')$arResult["INFO"]["STATUS"][$id]['NAME']='Placed';
