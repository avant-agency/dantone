...
elseif(count($arItems)==2):
	...
	if($arResult['IBLOCK_SECTION_ID']){
		...
		$arResult['RECOMMENDS'] = $arResult['RECOMMENDS_ITEMS'] =array();
		...
		while($arSection = $nav->GetNext()){
			$arResult['RECOMMENDS']=array_merge($arResult['RECOMMENDS'], $arSection['UF_RECOMMENDS']);
			$arResult['RECOMMENDS_ITEMS']=array_merge($arResult['RECOMMENDS_ITEMS'], $arSection['UF_RECOMMENDS_ITEMS']);
		}
	...
	$this->__component->SetResultCacheKeys(array('NAME', 'DETAIL_TEXT', 'SECTION', 'RECOMMENDS', 'RECOMMENDS_ITEMS'));