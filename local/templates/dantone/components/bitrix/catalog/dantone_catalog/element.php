...
$path = explode("/",$arResult["VARIABLES"]["SECTION_CODE_PATH"]);

foreach($path as $section_code) {

	$arFilter = Array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $section_code);
	$arSelect = Array("ID", "NAME", "IBLOCK_ID", "CODE", "SECTION_PAGE_URL", 'UF_ENG_NAME');
	$rsSection = CIBlockSection::GetList(Array(), $arFilter, false, $arSelect );
	while($arSection = $rsSection->GetNext()) {

		$APPLICATION->AddChainItem($arSection["UF_ENG_NAME"]&&LANGUAGE_ID=='en'?$arSection["UF_ENG_NAME"]:$arSection["NAME"], $arSection["SECTION_PAGE_URL"]); 
	}
}

$APPLICATION->IncludeComponent("bitrix:breadcrumb","catalog_breadcrumb",Array(
        "START_FROM" => "0", 
