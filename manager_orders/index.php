...
<section id="content">
	<div class="container">
		<h1 class="h2 mb10">Оформление заказа</h1>
		...
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"PATH_TO_AUTH" => "/auth/",
				"PATH_TO_BASKET" => "empty_basket.php",
...
<script>
	...
	setInterval(function(){
		order_price = $('#allSum_FORMATED').text().replace(/[ руб.]/g,'');
		$("#ORDER_PROP_5").val($("#LOCATION_val").val());
		
		if($("#ORDER_PROP_5").val().search("Московская область") != -1 || $("#ORDER_PROP_5").val().search("Москва") != -1 || $("#ORDER_PROP_5").val().search("москва") != -1){
